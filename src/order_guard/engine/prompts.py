...
## 工具调用策略
- 数据查询：优先使用 query_data（自然语言查数据，内部自动处理 schema 和 SQL）
- 只有在 query_data 失败或需要精细控制 SQL 时，才用 get_schema + query 组合
- 创建规则前：先用 get_schema 了解数据结构，确保 prompt_template 引用真实的表和字段
...
- 删除操作分级确认：
  - 删除单条或指定几条规则：告知用户要删哪些，用户同意后立即执行，只确认一次
  - 删除全部规则（scope=all）：需要二次确认。第一次告知总数和影响，用户同意后再次明确提醒"即将删除全部 N 条规则，此操作不可恢复"，用户再次确认后才执行
- 用户明确说"帮我创建"/"直接做"/"全部创建"/"确认"等指令 = 已授权，直接执行
- 不要在一个操作中反复确认，每个确认环节只问一次
...
_SCHEMA_INJECTED_SECTION = """\
## 已注入数据库 Schema（无需调用 list_datasources / get_schema）

以下是已连接数据源的表结构，已预加载至本次会话：

{schema_context}

**重要**: 上方 Schema 已完整注入，请直接使用 query 工具查询数据。
数据源 ID 和表字段见上方列表，无需再调用 list_datasources 或 get_schema。
"""


def build_unified_prompt(business_context: str = "", schema_context: str = "") -> str:
    """Build the unified system prompt with business context and optional schema injected."""
    ctx_section = ""
    if business_context:
        ctx_section = f"\n{business_context}\n"
    ...
    schema_section = ""
    if schema_context:
        schema_section = _SCHEMA_INJECTED_SECTION.format(schema_context=schema_context)

    return UNIFIED_SYSTEM_PROMPT.format(
        business_context=ctx_section,
