...
| 功能 | 说明 | 状态 |
|------|------|------|
| 统一 AI Agent | 19 个工具，一个 Agent 处理所有请求（查询、规则、告警、报告） | ✅ 已实现 |
| 多数据源接入 | 通过 MCP 协议连接 MySQL / PostgreSQL / SQLite，支持多库同时查询 | ✅ 已实现 |
| 自然语言查数据 | 描述需求 → AI 自动生成 SQL → 查询 → 返回分析结果 | ✅ 已实现 |
| 自然语言配规则 | 对话描述监控需求 → AI 理解数据结构 → 生成规则 → 确认生效 | ✅ 已实现 |
| 定时异常检测 | Cron 定时执行规则 → AI 分析数据 → 异常自动推送告警 | ✅ 已实现 |
| 飞书 Bot 对话 | @机器人查数据、管规则、看告警，支持多轮对话和会话管理 | ✅ 已实现 |
| 告警推送 | 飞书消息卡片 / 通用 Webhook，支持去重、静默期、批量合并 | ✅ 已实现 |
| 定时经营报告 | 日报 / 周报自动生成，自定义章节和 KPI 指标 | ✅ 已实现 |
| Schema 防幻觉 | 自动注入表结构到 AI 上下文，敏感表/字段黑名单过滤 | ✅ 已实现 |
| 查询审计 | 记录所有 AI 执行的 SQL，可追溯、可统计 | ✅ 已实现 |
| LLM 用量追踪 | Token 消耗、成本估算、按规则/模型分组统计 | ✅ 已实现 |
| 数据源健康监控 | 定时探活，连续失败自动告警，24h 可用率统计 | ✅ 已实现 |
| 告警闭环 | 告警可标记处理状态（已处理/忽略/误报），统计面板 | ✅ 已实现 |
| 规则效果评估 | 触发次数、误报率、执行成功率，智能调优建议 | ✅ 已实现 |
| 业务知识注入 | 配置公司特有业务上下文，AI 分析更懂业务 | ✅ 已实现 |
| CLI 管理工具 | 15+ 命令覆盖规则、告警、报告、会话、查询审计 | ✅ 已实现 |
| Docker 一键部署 | Dockerfile + docker-compose.yml，开箱即用 | ✅ 已实现 |
| 飞书文档 MCP 连接 | 自动同步飞书表格/文档中的业务上下文 | 📋 计划中 |
| Google Sheet 连接 | 读取促销日历、备货计划等运营表格 | 📋 计划中 |
| 企业微信 Bot | 复用统一 Agent，支持企微双向对话 | 📋 计划中 |
| Web 管理界面 | Dashboard、规则可视化、告警历史 | 📋 计划中 |
